...
export function PDFExportBOQ({
  ...
  // ⚠️ Performance warning for large documents
  if (boqItems.length > 200) {
    log.warn(`⚠️ BOQ has ${boqItems.length} items. This may take longer to render.`);
  }
  if (boqItems.length > 500) {
    log.warn(`🚨 BOQ has ${boqItems.length} items. Consider splitting into multiple documents.`);
  }
