...
export function NavigationMenu({
    ...
    try {
      ...
      if (cachedProfile && cachedMembership) {
        try {
          ...
          log.debug('⚡ NavigationMenu loaded from cache (instant)');
          
          // Still refresh in background to keep data fresh
          refreshProfileInBackground();
          return;
        } catch (e) {
          log.warn('Failed to parse cached profile in NavigationMenu');
        }
      }
      ...
      log.error('Failed to load user data in NavigationMenu:', error);
    }
  };
  ...
  const refreshProfileInBackground = async () => {
    ...
    try {
      ...
      if (!data || data.error) {
        log.debug('⚠️ NavigationMenu: Profile API returned error:', data?.error || 'Unknown error');
        return;
      }

      // api.get returns JSON data directly, not a Response object
      ...
      log.debug('🔄 NavigationMenu profile refreshed');
    } catch (error) {
      log.debug('NavigationMenu: Background refresh failed (not critical)');
    }
  };
