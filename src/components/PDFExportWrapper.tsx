...
import { BOQItem, Profile, CompanyInfo, CustomerInfo, Discount, PaymentTerms, BankInfo, TaxInvoice, BOQSummary } from '../types/boq';
...
export function PDFExportWrapper({
  ...
  // Helper: Calculate amount from term (support both percentage and amount)
  const calculateTermAmount = (term: PaymentTerm | null | undefined): number => {
    ...
    if (term.amount !== undefined && term.amount > 0) return term.amount;
    if (term.percentage !== undefined && term.percentage > 0) {
