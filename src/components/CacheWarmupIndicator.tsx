...
export function CacheWarmupIndicator() {
  ...
  useEffect(() => {
    ...
    if (!hasCriticalCache || stats.size < 8) {
      ...
      setWarming(true);
      console.log('🔥 Cache warmup indicator: Starting...');
      ...
      const checkInterval = setInterval(() => {
        ...
        if (hasAnalyticsNow && newStats.size >= 8) {
          setWarming(false);
          setCompleted(true);
          console.log('✅ Cache warmup complete! Analytics ready.');
      ...
      setTimeout(() => {
        clearInterval(checkInterval);
        if (warming) {
          console.log('⚡ Cache warmup timeout - continuing in background');
          setWarming(false);
        }
      }, 10000);
      ...
      console.log('✅ Cache already warm! Analytics available.');
    }
  }, []);
