...
export function CustomersPage({ onBack }: CustomersPageProps) {
  ...
  useEffect(() => {
    ...
    try {
      const raw = getUserLocalStorage('cache-customers');
      if (raw && isMounted) {
    ...
    const unsubscribe = api.cache.subscribe('customers', (data) => {
      if (!isMounted) return; // 🔒 Guard: Don't update if unmounted
      setCustomers(data.customers || []);
      // sync เก็บกลับ localStorage ต่อ-ยูส
      try { 
        setUserLocalStorage('cache-customers', JSON.stringify(data)); 
      } catch {}
    });
  ...
  const loadCustomers = async () => {
    ...
    try {
      ...
      // 🔒 DEBUG: Show current cache key for verification
      console.log('🔍 Loading customers with cache key:', getUserLocalStorage('cache-customers') ? 'HAS_CACHE' : 'NO_CACHE');
            ...
            <div className="flex items-center gap-2 sm:gap-3 flex-1 min-w-0">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xs sm:text-sm text-muted-foreground truncate">
                  จัดการข้อมูลลูกค้าและประวัิการทำงาน
                </p>
              </div>
            </div>
          </div>
