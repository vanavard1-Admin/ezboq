...
export function ReceiptPage({
  ...
  const calculateSummary = (): BOQSummaryType & { discountAmount: number; totalAfterDiscount: number } => {
    const subtotalMaterial = boqItems.reduce((sum, item) => sum + (item.material * item.quantity), 0);
    const subtotalLabor = boqItems.reduce((sum, item) => sum + (item.labor * item.quantity), 0);
              ...
              {selectedInstallment && paymentTerms ? (
                <>
                  ...
                  <tr style={{ backgroundColor: '#fed7aa' }}>
                    <td colSpan={5} style={{ border: '1px solid #000', padding: '10px', textAlign: 'right', fontSize: '12px', color: '#c2410c' }}>คงเหลือ</td>
          ...
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex gap-2">
              <Button onClick={handleSaveDocument} disabled={isSaving} className="gap-2">
                ...
                {isSaving ? "กำลังบันทึก..." : isSaved ? "✓ บันทึกแล้ว" : "บันทึกเอกสาร"}
              </Button>
