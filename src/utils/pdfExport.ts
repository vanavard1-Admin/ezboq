...
const ensureThaifontsLoaded = async (): Promise<void> => {
  ...
  try {
    ...
    log.warn('Failed to load Thai fonts:', error);
  }
};
...
const renderToCanvas = async (
  ...
  log.debug(`🎨 Rendering element: ${elementId}`);
  log.debug(`📐 Element dimensions: ${element.offsetWidth}x${element.scrollHeight || element.offsetHeight}`);
  ...
  log.debug(`⚠️ Found ${images.length} media elements in ${elementId} (will be removed)`);
  ...
  const renderPromise = html2canvas(element, {
    ...
    ignoreElements: (element) => {
      ...
      if (htmlEl.hasAttribute && htmlEl.hasAttribute('data-pdf-keep')) {
        return false; // Don't ignore
      }
      
      // Ignore all other media elements
    ...
    onclone: (clonedDoc) => {
      ...
      problematicElements.forEach((el) => {
        ...
        if (htmlEl.hasAttribute && htmlEl.hasAttribute('data-pdf-keep')) {
          return; // Skip this element, keep it as-is
        }
      ...
      if (clonedElement) {
        log.debug(`✅ Found cloned element: ${elementId}`);
        ...
        while (currentParent && depth < 5) {
          if (currentParent.hasAttribute('data-pdf-export-wrapper')) {
            log.debug(`✅ Found PDF export wrapper parent, resetting position`);
  ...
  try {
    log.debug(`⏱️ Starting render with ${PDF_CONFIG.maxRenderTime / 1000}s timeout...`);
    canvas = await Promise.race([renderPromise, timeoutPromise]);
    log.debug(`✅ Render completed within timeout`);
  } catch (timeoutError) {
    log.error(`❌ Render timeout or error:`, timeoutError);
    throw timeoutError;
  }
  ...
  log.debug(`✅ Canvas rendered successfully: ${canvas.width}x${canvas.height}`);
  return canvas;
};
...
export const exportToPDF = async (
  ...
  try {
    log.debug(`📄 Starting PDF export for: ${elementId}`);
    
    // Debug: List all export sections available
    const allExportSections = Array.from(document.querySelectorAll('[id*="export-section"]'));
    log.debug(`🔍 Available export sections (${allExportSections.length}):`, allExportSections.map(el => ({
      id: el.id,
    ...
    if (!element) {
      ...
      log.debug('💡 Tip: Check that PDFExportWrapper is rendered in the DOM');
      throw new Error(`Element "${elementId}" not found`);
    }

    log.debug(`✅ Element found: ${elementId}`);
    log.debug(`   📐 Dimensions: ${element.offsetWidth}x${element.offsetHeight}`);
    log.debug(`   🎨 Computed style:`, {
    ...
    try {
      ...
      log.debug(`📸 Converting main canvas to data URL (${canvas.width}x${canvas.height})...`);
      let imgData: string;
      try {
        ...
        try {
          ...
          log.debug(`✅ Canvas is not tainted`);
        } catch (taintError) {
          log.error(`⚠️ Canvas may be tainted:`, taintError);
        }
        
        imgData = canvas.toDataURL(PDF_CONFIG.mimeType, PDF_CONFIG.imageQuality);
        log.debug(`✅ Main canvas converted to data URL successfully (${imgData.length} bytes)`);
      } catch (error) {
        log.error(`❌ Failed to convert canvas to data URL:`, error);
      ...
      log.debug(`📄 Document will be split into ${totalPages} page(s)`);
      log.debug(`📐 Canvas: ${canvasWidthMM.toFixed(2)}mm x ${canvasHeightMM.toFixed(2)}mm`);
      log.debug(`📐 Scaled: ${scaledWidth.toFixed(2)}mm x ${scaledHeight.toFixed(2)}mm`);
      ...
      if (totalPages === 1) {
        ...
        for (let page = 0; page < totalPages; page++) {
          ...
          if (pageCtx) {
            ...
            log.debug(`🎨 Drawing page ${page + 1} content from main canvas...`);
            log.debug(`   Source: y=${((sourceY / canvasHeightMM) * canvas.height).toFixed(0)}, h=${pageCanvas.height}`);
            log.debug(`   Target: ${pageCanvas.width}x${pageCanvas.height}`);
            ...
            try {
              ...
              if (sourceYPixels < 0 || sourceYPixels >= canvas.height) {
                log.error(`❌ Invalid source Y position: ${sourceYPixels} (canvas height: ${canvas.height})`);
              ...
              try {
                ...
                log.error(`❌ Main canvas is tainted, cannot draw to page canvas:`, taintCheck);
              ...
              log.warn(`⚠️ Creating blank page ${page + 1} due to draw error`);
            ...
            try {
              log.debug(`📸 Converting page ${page + 1} canvas to data URL (${pageCanvas.width}x${pageCanvas.height})...`);
              
              // Test if page canvas is tainted
              try {
                pageCtx.getImageData(0, 0, 1, 1);
                log.debug(`✅ Page ${page + 1} canvas is not tainted`);
              } catch (taintError) {
                log.error(`⚠️ Page ${page + 1} canvas may be tainted:`, taintError);
              }
              ...
              try {
                ...
                log.debug(`✅ Page ${page + 1} canvas converted as JPEG (${pageImgData.length} bytes)`);
              } catch (jpegError) {
                log.warn(`⚠️ JPEG export failed for page ${page + 1}, trying PNG:`, jpegError);
                try {
                  // Fallback to PNG
                  pageImgData = pageCanvas.toDataURL('image/png');
                  exportFormat = 'PNG';
                  log.debug(`✅ Page ${page + 1} canvas converted as PNG (${pageImgData.length} bytes)`);
                } catch (pngError) {
                  log.error(`❌ Both JPEG and PNG export failed for page ${page + 1}`);
      ...
      if (isMobileDevice()) {
        log.debug('📱 Mobile device detected, using mobile-friendly download');
      ...
      log.debug(`✅ PDF saved successfully: ${finalFilename}`);
    } finally {
      restore();
    }
  } catch (error: any) {
    log.error(`❌ PDF export failed for ${elementId}:`, error);
...
export const exportWorkflowToPDF = async (
  ...
  log.debug(`📦 Starting workflow PDF export: ${documentType} (${total} documents)`);
  ...
  if (documentType === 'boq' || documentType === 'all') {
    ...
    if (boqElement) {
      ...
      if (itemCount > 200) {
        log.warn(`⚠️ BOQ contains ${itemCount} items. Export may take 30-60 seconds.`);
      }
      if (itemCount > 500) {
        log.warn(`🚨 BOQ contains ${itemCount} items. This is a large document - please wait patiently.`);
      }
    }
  }
  ...
  try {
    ...
    try {
      ...
      log.debug(`📄 Processing document ${i + 1}/${total}: ${doc.title}`);
      ...
      successCount++;
      log.debug(`✅ Successfully exported: ${doc.title}`);
  ...
  if (successCount === 0) {
    ...
    log.debug(`✅ Workflow export complete: ${successCount}/${total} documents exported successfully`);
  } finally {
