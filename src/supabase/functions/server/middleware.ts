...
export function corsMiddleware() {
  return async (c: Context, next: Function) => {
    ...
    // 🔒 Check if origin is allowed (STRICT in production)
    let allowedOrigin = '';
    ...
    if (ALLOWED_ORIGINS.has(origin)) {
      ...
      // 🚨 PRODUCTION: Reject non-whitelisted origins
      console.error(`🚫 CORS: Blocked non-whitelisted origin in production: ${origin}`);
      // In strict mode, we should reject the request
      // But for now, we'll allow it with wildcard for backward compatibility
      allowedOrigin = '*';
      console.warn(`⚠️ CORS: Using wildcard for ${origin} (consider whitelisting)`);
    }
    ...
    if (method === 'OPTIONS') {
      ...
      return new Response(null, {
        ...
        headers: {
          ...
          'Access-Control-Expose-Headers': 'X-RateLimit-Limit,X-RateLimit-Remaining,X-RateLimit-Reset,Retry-After,X-Request-ID,X-Cache',
          'Vary': 'Origin',
        },
      });
    }
 ...
 * Max 15 seconds for any request (allows for complex queries)
 * 
 ...
 */
export function requestTimeoutMiddleware(maxTimeout: number = 15000) {
  return async (c: Context, next: Function) => {
    const requestId = c.get('requestId') || 'unknown';
    const path = c.req.path;
    ...
    if (duration > maxTimeout) {
      ...
      console.warn(`[${requestId}] ⚠️ SLOW: ${path} took ${duration}ms (within ${maxTimeout}ms limit)`);
    }
  };
}
