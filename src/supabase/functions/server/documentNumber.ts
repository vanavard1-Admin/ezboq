...
const MAX_RETRIES = 5; // Reduced retries for faster failure
const INITIAL_BACKOFF = 10; // milliseconds - faster backoff
const KV_TIMEOUT = 1500; // 1.5 seconds timeout for KV operations (optimized for speed)
const LOCK_TIMEOUT = 2000; // 2 seconds max wait time for lock acquisition (optimized for speed)
 ...
 * 
 * ⚡ OPTIMIZED: Uses dedicated index instead of full table scan
 */
export async function validateDocumentNumberUnique(
  ...
  // ⚡ FAST PATH: Check dedicated document number index
  const indexKey = getKeyPrefix(context, `docnum_index:${docNumber}`);
  
  try {
    const indexed = await withTimeout(
      kv.get(indexKey),
      500, // Fast timeout for index lookup
      'docnum_index_lookup'
    );
    
    if (indexed && indexed !== documentId) {
      const elapsed = Date.now() - startTime;
      console.log(`[${requestId}] ⚠️ Duplicate doc number found in ${elapsed}ms (index): ${docNumber}`);
      return {
        valid: false,
        error: `Document number ${docNumber} is already in use`
      };
    }
    
    const elapsed = Date.now() - startTime;
    console.log(`[${requestId}] ✅ Doc number unique check passed in ${elapsed}ms (index): ${docNumber}`);
    return { valid: true };
    
  } catch (error: any) {
    console.error(`[${requestId}] ❌ Index lookup failed, fallback to scan:`, error.message);
    
    // FALLBACK: Full scan if index fails (slower but safe)
