...
export function HistoryPage({ onBack, onEditDocument }: HistoryPageProps) {
  ...
  const loadDocuments = async () => {
    ...
    try {
      ...
      if (response?.ok) {
        ...
        console.log(`✅ Documents loaded in ${duration}ms`);
      } else {
  ...
  // ✅ นับมูลค่ารวมเฉพาะเอกสารที่อนุมัติแล้ว
  // สำหรับใบเสนอราคา: นับเฉพาะ status = approved/completed/paid
  // สำหรับเอกสารอื่นๆ: นับทุก status ยกเว้น draft และ cancelled
  const totalAmount = filteredDocuments.reduce((sum, d) => {
    // ใบเสนอราคา: ต้องอนุมัติแล้วเท่านั้น
    if (d.type === 'quotation') {
        ...
        <DialogContent>
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="bg-blue-50 p-3 rounded-lg border border-blue-200">
              <p className="text-sm text-blue-700">
                💡 <strong>ทางเลือ���อื่น:</strong> คุณสามารถ "เปลี่ยนสถานะเป็นยกเลิก" แทนการลบได้
              </p>
            </div>
          </div>
